...
const toNumber = (val) => {
  let transformed = _.toNumber(val)
  if(!_.isNaN(transformed))
    return transformed
  return val
}

const typedValues = (obj) => {
  let keys = Object.keys(obj)
  let ret = _.clone(obj)

  keys.forEach(key => {
    ret[key] = toTrueFalse( toNumber( obj[key] ) )
  })
...
const search = (opts, resources) => {
  ...
  return _.find(resources, typedValues(opts))
}
...
const filter = (opts, resources) => {
  ...
  return _.filter(resources, typedValues(opts) )
}
...
async function KeycloakAPI({url, realm, id}) {
  ...
  let URLs = {
    ...
    clients:     (host, realm) =>  `https://${host}/auth/admin/realms/${realm}/clients`,
    ...
    realm:     (host, realm) =>  `https://${host}/auth/admin/realms`,
