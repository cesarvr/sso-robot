...
const loadTemplates = ({name, file}) => { return { name, file: JSON.parse( require('fs').readFileSync(file).toString() ) } }
const configureTemplateNames = (tname, {name, file}) => { file.metadata.name = tname; return { name, file }  }
