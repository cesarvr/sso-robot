...
async function getAdminAccess({url, username, password}) {
  let AUTH_HOST = url
  ...
  let oauth2_info = await discover(AUTH_HOST, 'master')
...
module.exports = {
  run: async function(url, username, password, realm){
    let token = await getAdminAccess({url, username, password, realm})
